function [x,out] = SPPDG_SARAH_f(opts,L,A,b,K)
%
% SPPDG_SARAH_f is the code of SPPDG with SARAH proposed in the paper "Preconditioned Primal-Dual 
...
for iter = 1:maxit
    ...
    cache1 = (tanh(b.*(A*x_0)).^2-1).*b;  
    full_grad        = (cache1'*A)'/N;    
     
    cache1 = (tanh(b.*(A*x_)).^2-1).*b;  
    full_grad_        = (cache1'*A)'/N;   
    ...
    if   p_t == 1
         ...
         cache1   =  (tanh(b_sample_g.*(A_sample_g*x_0)).^2-1).*b_sample_g;  
         g1       = (cache1'*A_sample_g)'/ssize; 
          
         cache2 =  (tanh(b_sample_g.*(A_sample_g*x_)).^2-1).*b_sample_g;  
         g2     = (cache2'*A_sample_g)'/ssize; 
         grad   = g1 - g2 + full_grad_ ;
    end
    end
