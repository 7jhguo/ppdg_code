function [paramx, options] = PPDG_SVRG(model,X, y,X_test, y_test, options)
% PPDG_SVRG is a code for image classification on 1-hidden layer network
% using SPPDG with SVRG proposed in paper: Preconditioned Primal-Dual 
...
for iter = cur+1: cur + max_iters
 ...
 %%  inner loop  
  for t =1:m  
   ...
   noProps = noProps + size(x_sample,2);
   grad  = grad1-grad2+fullgrad;  % svrg estimator
      
   %%  
   paramz      = paramx_0- alpha *(grad+paramy);   
