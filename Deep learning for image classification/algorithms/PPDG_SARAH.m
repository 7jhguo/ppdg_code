function [paramx, options] = PPDG_SARAH(model,X, y,X_test, y_test, options)
% PPDG_SARAH is a code for image classification on 1-hidden layer network
% using SPPDG with SARAH proposed in paper: Preconditioned Primal-Dual 
...
for iter = cur+1: cur + max_iters
   ...
   paramz_1 =  paramx_0- alpha *(fullgrad+paramy_0); 
   paramy_temp_1 = paramy_0+(2*paramz_1-paramx_0)/alpha; 
...
%%  inner loop
 for  initer=1:m
       ...
       noProps    = noProps + size(x_sample,2);
       grad       =  grad2-grad3+grad1; % sarah estimator
      
       %% 
       paramz     = paramx_1- alpha *(grad+paramy_1);    
       paramy_temp = paramy_1+(2*paramz-paramx_1)/alpha; 
