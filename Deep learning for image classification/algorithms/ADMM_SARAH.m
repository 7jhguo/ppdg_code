function [paramx, options] = ADMM_SARAH(model,X, y,X_test, y_test, options)
% ADMM_SARAH is a code for image classification on 1-hidden layer network
% using SADMM with SARAH.
%
%
% input & ouput: 
...
if isfield(options,'params')
  ...
  paramy = options.params;
  paramz = options.params;
else
  paramx = sprandn(psize,1,0.1)*0.5;
  paramy = sprandn(psize,1,0.1)*0.5;
  paramz = sprandn(psize,1,0.1)*0.5;
end
...
for iter = cur+1: cur + max_iters
   ...
   paramx_1 =  paramx_0- alpha *(fullgrad+paramz_0+1/alpha*(paramx_0-paramy_0)); 
   paramy_temp_1 = paramx_1+alpha*paramz_0; 
   paramy_1 = 1/max(1,norm(paramy_temp_1,inf)/lambda)*paramy_temp_1;  
   paramz_1  =  paramz_0 + 1/alpha*(paramx_1-paramy_1);
 ...
 for  initer=1:2
       ...
       %%
       paramx     = paramx_1- alpha *(grad+paramz_1+1/alpha*(paramx_1-paramy_1));    %%%%% main loop
       paramy_temp = paramx+alpha*paramz_1; 
       %l1 regularizer
       paramy = 1/max(1,norm(paramy_temp,inf)/lambda)*paramy_temp;
  
       paramz  =  paramz_1 + 1/alpha*(paramx-paramy);
       paramy_1  =  paramy;
       paramx_1  =  paramx;
       paramz_1  =  paramz;
