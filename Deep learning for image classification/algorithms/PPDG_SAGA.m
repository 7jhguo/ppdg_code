function [paramx, options] = PPDG_SAGA(model,X, y,X_test, y_test, options)  
% PPDG_SAGA is a code for image classification on 1-hidden layer network
% using SPPDG with SAGA proposed in paper: Preconditioned Primal-Dual 
% Gradient Methods for Nonconvex Composite and Finite-Sum Optimization. 
%
%
% input & ouput: 
...
for iter = cur+1: cur + max_iters
   ...
   [~, grads] = compute_model(model, paramx, x_sample, y_sample);
   noProps    = noProps + size(x_sample,2);
   
   [~, grads_] = compute_model(model, G(:,mod(iter,2)+1), x_sample, y_sample);          
   noProps     = noProps + size(x_sample,2);
    
   [~, grad] = compute_model(model, G(:,mod(iter,2)+1), X, y);  
   fullgrad  = grad;
   noProps   = noProps + size(X,2);
    
   grad  = grads-grads_+fullgrad;
   
   %%  iteration
   paramz      = paramx- alpha *(grad+paramy);   
   paramy_temp = paramy+(2*paramz-paramx)/alpha;
   paramy      = 1/max(1,norm(paramy_temp,inf)/lambda)*paramy_temp; %l1 regularizer
   paramx      = paramz; 
   G(:,mod(iter,2)+1) =  paramx;
