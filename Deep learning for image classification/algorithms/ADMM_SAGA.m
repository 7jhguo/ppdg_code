function [paramx, options] = ADMM_SAGA(model,X, y,X_test, y_test,options)
% ADMM_SAGA is a code for image classification on 1-hidden layer network
% using SADMM with SAGA.
%
%
% input & ouput: 
...
for iter = cur+1: cur + max_iters
   ...
   grad  = grads-grads_+fullgrad;
    ...
    %%  iteration
   paramx = paramx- alpha *(grad+paramz+1/alpha*(paramx-paramy));  
   paramy_temp = paramx+alpha*paramz;
   %%l1 regularizer   
   paramy = 1/max(1,norm(paramy_temp,inf)/lambda)*paramy_temp;  
   paramz = paramz + 1/alpha*(paramx-paramy); 
